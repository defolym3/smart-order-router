...
import { Protocol } from '@defolym3/router-sdk';
import { ChainId, Currency, CurrencyAmount, Token, TradeType } from '@defolym3/sdk-core';
import { FeeAmount, Pool } from '@defolym3/v3-sdk2';
import { Pair } from '@defolym3/v2-sdk/dist/entities';
...
export async function calculateGasUsed(
  ...
  //   )[1];
  // } else if ([ChainId.OPTIMISM, ChainId.BASE, ChainId.BASE_SEPOLIA].includes(chainId)) {
  ...
  if ([ChainId.KAIROS, ChainId.SEPOLIA, ChainId.OPTIMISM, ChainId.BASE, ChainId.BASE_SEPOLIA, ChainId.MODE].includes(chainId)) {
    l2toL1FeeInWei = calculateOptimismToL1FeeFromCalldata(
      route.methodParameters!.calldata,
      l2GasData as OptimismGasData
    )[1];
